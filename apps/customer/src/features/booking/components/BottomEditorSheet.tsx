...
export function BottomEditorSheet({ children, isOpen, onClose, title }: BottomEditorSheetProps) {
  useEffect(() => {
    ...
    <Portal>
      ...
      >
        ...
        >
          <Stack
            direction="row"
            sx={{ alignItems: 'center', justifyContent: 'space-between', gap: 1.5, borderBottom: '1px solid', borderColor: 'divider', p: 1.5 }}
          >
            <Typography component="h2" sx={{ fontSize: '1.1rem', fontWeight: 950 }}>
              {title}
            </Typography>
          ...
          <Box sx={{ minHeight: 0, overflowY: 'auto', p: 2.5 }}>{children}</Box>
        </Box>
      </Box>
    </Portal>
  )
